...
여기서 주의해야할 점은 `use client` 선언을 통해 클라이언트 컴포넌트가 되었다고 해당 컴포넌트는 클라이언트 사이드 렌더링만 된다는 의미가 아니다. 처음에 이 클라이언트 컴포넌트 개념이 굉장히 헷갈렸는데, Next.js에서는 `use client` 선언 여부 상관 없이 일단 모든 컴포넌트가 서버 사이드 렌더링이 되며, 이후 `use client` 선언이 된 클라 컴표넌트는 **hydration**을 통해 리액트 컴포넌트가 되어야 하는 코드가 있다는 것을 의미한다.
...
| 변경 전                                                         | 변경 후                                       |
| --------------------------------------------------------------- | --------------------------------------------- |
| `getServerSideProps()`, `getStaticProps()`, `getInitialProps()` | `fetch(주소, 두번째 인자 설정)` (하나로 통일) |
