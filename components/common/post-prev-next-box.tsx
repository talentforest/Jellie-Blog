...
export default function PostPrevNextBox({ post, direction }: PostBoxProps) {
  ...
  return (
    <Link
      ...
      } w-full md:w-[60vw] h-full overflow-hidden relative flex flex-row group hover:-translate-y-2 hover:border-2 border-light-yellow rounded-2xl transition`}
    >
      <div
        className={`${
      ...
      >
        ...
        <h2
          className={`flex-1 mt-2.5 mb-3 min-h-10 text-base font-bold group-hover:text-yellow`}
        >
          {title}
        </h2>
      ...
      {thumbnail ? (
        <Image
          ...
          height={200}
          priority
          className={`max-h-full object-cover border w-1/3 ${
